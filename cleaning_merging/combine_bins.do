...
*** Set cutoffs, increment, and unit
** Bottom bin
* lower bound
local bot_lb "nInf"
* upper bound
...
local `init' = `bot_ub' - `inc'

forvalues ii = `init'(`inc')`top_lb' {
	* Store bin lower and upper bounds
	local lb = `ii'
	local ub = `ii' + `inc'
	
	* Store cutoffs as formatted string
	if `ii' == `init' {
		local cutoffs "`bot_lb'_`ub'`unit'"
	}
	else if `ii' == `top_lb' {
		local cutoffs "`lb'`unit'_`top_ub'"
	}
	else {
		local cutoffs "`lb'`unit'_`ub'`unit'"
	}
	* replace dash with "n" for negatives
	local cutoffs = subinstr("`cutoffs'", "-", "n", .)
	...
	*foreach weight in area pop crop {
	foreach weight in pop {
		...
		bysort `adminVar' (year): gen `bin13' = `bin' + tavg_bin_`cutoffs'_m12_`weight'[_n-1]
