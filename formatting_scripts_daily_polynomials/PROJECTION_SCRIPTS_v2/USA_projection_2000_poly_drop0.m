...
outputDir = [sample '_daily_' CLIM '_polynomials_drop0'];
...
lat = flip([precip_poly_1.lat(1:90)-0.5; precip_poly_1.lat(92:end)+0.5]);
...
elseif T == 4
    ...
    command = ['temp_actual = cat(2, precip_poly_' num2str(power) '.precip_actual_poly(:,ixlon:end,:), precip_poly_' num2str(power) '.precip_actual_poly(:,1:ixlon-1,:));'];
    eval(command)
    command = ['temp_actual = flip(temp_actual(1:end ~= 91,:,:), 1);'];
    eval(command)
    var = 'precip';
end
